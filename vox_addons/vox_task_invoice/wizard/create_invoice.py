...
class SaleTaskPaymentInv(models.TransientModel):
    _name = "sale.task.payment.inv"
    _description = "Sales Task Invoice"
    ...
    def _default_currency_id(self):
        if self._context.get('active_model') == 'project.task' and self._context.get('active_id', False):
            task = self.env['project.task'].browse(self._context.get('active_id'))
            return task.sale_id.currency_id
    ...
    def _create_invoices_vals(self):
        ...
        # sale_obj.write(sale_id, {'invoice_ids': [(4, inv_id)]})
        # return inv_id
