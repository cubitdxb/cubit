...
class SaleOrder(models.Model):
    ...
    actual_cost_price_total = fields.Float(string='Actual Cost Price Total', store=True, readonly=True,
                                           compute='_compute_cost_price', copy=False)
    profit = fields.Float(string='Profit', store=True, readonly=True, compute='_compute_cost_price', copy=False, groups="vox_task_template.group_gross_profit_user,base.group_system")
                ...
                for line in order.order_line:
                    ...
                    if order.discount_distribution_type == 'against_item':
                        line.discount_distribution = ((order.service_discount_amount + order.discount_amount) /
                                                      qty_price_total) * (line.product_uom_qty * line.unit_price) if line.unit_price else 0.0
                    elif order.distribution_tax_ids and line.tax_id and line.tax_id.ids[0] in order.distribution_tax_ids.ids:
                        print("dist",order.distribution_tax_ids.ids)
                        line.discount_distribution = ((order.service_discount_amount + order.discount_amount) /
                                                      tax_wise_total) * (line.product_uom_qty * line.unit_price) if tax_wise_total else 0.0
                    else:
